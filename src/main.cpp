...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20);//start diff is 0.25
...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 20; // stake age of full weight
unsigned int nStakeTargetSpacing = 2 * 60; // 120 seconds block spacing
const int64 nChainStartTime = 1388357790; // 2013-08-10 8:00:00 GMT
...
const string strMessageMagic = "Battlecoin Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    // Battlecoin: reward for coin-year is cut in half every 64x multiply of PoS difficulty
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal Battlecoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Battlecoin
    bool fStrictPayToScriptHash = true; // Always active in Battlecoin
    ...
    // ppcoin: fees are not collected by miners as in Battlecoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Battlecoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Battlecoin has arrived";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:Battlecoin genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x466210afafdeaa4ff185b1a192771497fbd2e7cda3dd38ddf715a84442f513a4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x03, 0xe8, 0x03, 0xe4 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("BattlecoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("BattlecoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BattlecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("BattlecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadBattlecoinMiner(void* parg);

static bool fGenerateBattlecoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void BattlecoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("Battlecoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateBattlecoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateBattlecoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running BattlecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBattlecoinMiner(void* parg)
{
    ...
    try
    {
        ...
        BattlecoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadBattlecoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadBattlecoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadBattlecoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateBattlecoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateBattlecoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateBattlecoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d BattlecoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadBattlecoinMiner, pwallet))
                printf("Error: NewThread(ThreadBattlecoinMiner) failed\n");
            Sleep(10);
        }
    }
}
